...
@click.option(
    "-l", "--list", "list_sessions", is_flag=True, help="List recent sessions"
)
...
def _get_or_create_session(
    ...
    if session:
        ...
        if session_mgr.is_stale(session, config.session.timeout_hours):
            if not quiet:
                console.print(
                    f"[yellow]Session {session.id} is stale, starting fresh[/yellow]"
                )
            session = session_mgr.create(
                model=config.agent.model,
                repo_path=os.getcwd(),
            ...
            if not quiet:
                ...
                console.print(
                    f"[dim]Session: {session.id} ({msg_count} messages)[/dim]"
                )
    else:
...
async def _run_agent(
    ...
    if json_output:
        ...
        if not quiet:
            ...
            console.print(
                f"[dim]Tokens: {total_tokens} ({response.tokens_prompt}→{response.tokens_completion})[/dim]"
            )
...
def _list_sessions(session_mgr: SessionManager) -> None:
    ...
    for s in sessions:
        ...
        if s.messages:
            last = s.messages[-1]
            preview = (
                last.content[:40] + "..." if len(last.content) > 40 else last.content
            )
        ...
        console.print(
            f"{marker} [bold]{s.id}[/bold] {age_str} ({msg_count} msgs) - {preview}"
        )
...
def _show_recall(session_mgr: SessionManager) -> None:
    ...
    messages_to_show = (
        session.messages[-2:] if len(session.messages) >= 2 else session.messages
    )
