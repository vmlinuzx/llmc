...
## Error recovery
Tool errors return: {'error': 'message', 'meta': {'path': '...', 'allowed_roots': [...]}}
Use allowed_roots to correct paths. Retry with absolute path within allowed root.

## Token efficiency
- rag_query returns summaries - scan before expanding
- Only read_file on relevant hits
- Filter/process in execute_code, return minimal stdout
"""
...
class LlmcMcpServer:
    ...
    def __init__(self, config: McpConfig):
        self.config = config
        self.server = Server("llmc-mcp", instructions=BOOTSTRAP)
