...
from llmc_mcp.tools import te as te_mod
...
def test_repo_read_builds_args(monkeypatch):
    ...
    ctx = DummyCtx()
    res = te_repo.repo_read("/repo", "README.md", max_bytes=1024, ctx=ctx)
...
def test_rag_query_builds_args_and_filters(monkeypatch):
    ...
    monkeypatch.setattr(te_repo, "te_run", fake_te_run)
    res = te_repo.rag_query("howdy", k=3, index="default", filters={"lang":"py"})
