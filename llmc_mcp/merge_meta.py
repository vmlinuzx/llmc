...
class GraphPatch:
    ...
    properties_to_set: dict[str, dict[str, Any]] = field(
        default_factory=dict
    )  # {node_id: {key: value}}
    properties_to_clear: dict[str, list[str]] = field(
        default_factory=dict
    )  # {node_id: [keys]}
...
class MergeEngine:
    ...
    def _execute_merge(
        ...
        try:
            ...
            sorted_edges = sorted(
                patch.edges_to_add, key=lambda e: (e["source"], e["target"], e["type"])
            )
            for edge in sorted_edges:
                ...
                if target not in graph_store.adjacency[source]["outgoing"][edge_type]:
                    ...
                    if (
                        source
                        not in graph_store.adjacency[target]["incoming"][reverse_type]
                    ):
                        graph_store.adjacency[target]["incoming"][reverse_type].append(
                            source
                        )
                else:
                    conflicts.append(
            ...
            if conflicts:
                logger.warning(
                    f"Graph merge had {len(conflicts)} conflicts (using LWW)"
                )
