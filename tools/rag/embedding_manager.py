...
class EmbeddingManager:
    ...
    def _from_config(cls) -> EmbeddingManager:
        ...
        """
        try:
            from ..config import get_llmc_config  # type: ignore[attr-defined]
        except ImportError:
            try:  # pragma: no cover - fallback path
                from tools.config import get_llmc_config  # type: ignore[attr-defined]
            except ImportError as exc:
                raise EmbeddingConfigError(
                    "Could not import get_llmc_config. "
                    "Update tools/rag/embedding_manager.py to import "
                    "your configuration loader correctly."
                ) from exc
