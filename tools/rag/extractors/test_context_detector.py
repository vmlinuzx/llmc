import pytest
from tools.rag.extractors.context_detector import ContextDetector, detect_negation, detect_context
...
def test_detect_negation():
    ...
    assert result_func == True, f"Convenience function should also detect negation"
...
def test_detect_context_negation():
    ...
    assert result['negated'] == True, f"Expected 'chest pain' to be negated"
    assert result['historical'] == False, f"Should not be historical"
    assert result['family'] == False, f"Should not be family"
    assert result['hypothetical'] == False, f"Should not be hypothetical"
...
def test_detect_context_historical():
    ...
    assert result['historical'] == True, f"Expected 'diabetes' to be historical"
    assert result['negated'] == False, f"Should not be negated"
    assert result['family'] == False, f"Should not be family"
    assert result['hypothetical'] == False, f"Should not be hypothetical"
...
def test_detect_context_family():
    ...
    assert result['family'] == True, f"Expected 'cancer' to be family history"
    assert result['negated'] == False, f"Should not be negated"
    assert result['historical'] == False, f"Should not be historical (it's family, not patient's history)"
    assert result['hypothetical'] == False, f"Should not be hypothetical"
...
def test_detect_context_multiple_categories():
    ...
    assert result['negated'] == True, f"Expected 'diabetes' to be negated due to 'No'"
    assert result['family'] == True, f"Expected 'diabetes' to be family due to 'family history'"
    assert result['historical'] == False, f"Should not be historical (it's family history)"
    assert result['hypothetical'] == False, f"Should not be hypothetical"
...
def test_detect_context_hypothetical():
    ...
    assert result['hypothetical'] == True, f"Expected 'infection' to be hypothetical due to 'consider'"
    assert result['negated'] == False, f"Should not be negated"
    assert result['historical'] == False, f"Should not be historical"
    assert result['family'] == False, f"Should not be family"
...
def test_context_window():
    ...
    result = detector.detect_negation(text, entity_span)
    assert result == True, f"Should detect negation even with surrounding text"
...
def test_case_insensitivity():
    ...
    assert result == True, f"Should detect negation even with uppercase 'DENIES'"
