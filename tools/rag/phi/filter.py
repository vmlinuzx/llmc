...
from .detector import PHIDetector, PHIMatch
...
class PHIFilter:
            ...
            if type_ in redact_types:
                ...
                for date_format in ["%m/%d/%Y", "%Y/%m/%d", "%m-%d-%Y", "%Y-%m-%d"]:
                    try:
                        ...
                        replacement = self.date_shifter.shift_date(original, date_format)
                        break
                    except ValueError:
                        continue
    ...
    def redact_all(self, text: str) -> str:
        ...
        """
        return self.filter_text(text, redact_types=["SSN", "PHONE", "EMAIL", "DATE", "MRN", "IP", "NAME"])
