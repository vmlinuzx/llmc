...
def generate(
    ...
    all: bool = typer.Option(
        False, "--all", help="Generate docs for all indexed files"
    ),
    force: bool = typer.Option(
        False, "--force", help="Force generation (ignore SHA gate)"
    ),
):
    ...
    if not db_path:
        typer.echo(
            f"❌ RAG database not found. Searched: {[str(p) for p in candidates]}",
            err=True,
        )
    ...
    if all:
        ...
        if input_path.is_absolute():
            try:
                ...
                typer.echo(
                    "⚠️  Warning: Path appears to be outside repository root", err=True
                )
...
def status():
    ...
    typer.echo(
        f"Coverage:         {doc_count}/{total_files} ({100*doc_count//max(total_files,1)}%)"
    )
