...
def query(
    ...
    model: str = typer.Option(
        None, "--model", help="Override root model (e.g., deepseek/deepseek-reasoner)"
    ),
    ...
    dump_prompts: bool = typer.Option(
        False, "--dump-prompts", help="Dump full prompts to /tmp/rlm_prompts.log"
    ),
    ...
    """
    asyncio.run(_run_query(task, file, context, model, budget, trace, dump_prompts, json_output))
...
async def _run_query(
    ...
    if dump_prompts:
        ...
        os.environ["RLM_DUMP_PROMPTS"] = "/tmp/rlm_prompts.log"
        console.print("[yellow]Dumping prompts to /tmp/rlm_prompts.log[/yellow]")
    ...
    if not json_output:
        console.print(
            f"[green]Context loaded:[/green] {meta['total_chars']:,} chars, ~{meta['estimated_tokens']:,} tokens"
        )
...
def _display_result(result: RLMResult, show_trace: bool):
    ...
    if result.budget_summary:
        ...
        console.print(
            f"  Cost: ${b['total_cost_usd']:.4f} / ${b['budget_usd']:.2f} ({b['budget_used_pct']:.1f}%)"
        )
        console.print(f"  Root: {b['root_calls']} calls, ${b['root_cost_usd']:.4f}")
        console.print(
            f"  Sub: {b['sub_calls']} calls, ${b['sub_cost_usd']:.4f}, max depth {b['max_subcall_depth']}"
        )
    ...
    if show_trace and result.trace:
        ...
        for i, event in enumerate(result.trace):
            console.print(
                f"  {i + 1}. [{event['event']}] {json.dumps({k: v for k, v in event.items() if k not in ['event', 'timestamp', 'session_id']})}"
            )
