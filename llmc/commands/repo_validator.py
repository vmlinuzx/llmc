...
class ValidationResult:
    ...
    def add_error(self, category: str, message: str, fix_hint: str | None = None, file_path: str | None = None) -> None:
        self.add_issue(ValidationIssue("error", category, message, fix_hint, file_path))
    
    def add_warning(self, category: str, message: str, fix_hint: str | None = None, file_path: str | None = None) -> None:
        self.add_issue(ValidationIssue("warning", category, message, fix_hint, file_path))
    
    def add_info(self, category: str, message: str, fix_hint: str | None = None, file_path: str | None = None) -> None:
...
def load_config(repo_path: Path, result: ValidationResult) -> dict[str, Any] | None:
    ...
    if tomllib is None:
        result.add_error("config", "No TOML parser available (need Python 3.11+ or tomli package)")
        return None
...
def check_enrichment_config(config: dict[str, Any], result: ValidationResult) -> None:
    ...
    for i, chain in enumerate(enabled_chains):
        ...
        if not chain.get("provider"):
            result.add_warning("config", f"Chain '{chain_name}' missing 'provider' field")
    ...
    result.add_info("config", f"Found {len(enabled_chains)} enabled enrichment chain(s)")
...
def check_embeddings_config(config: dict[str, Any], result: ValidationResult) -> None:
    ...
    result.add_info("config", f"Found {len(profiles)} embedding profile(s), {len(routes)} route(s)")
...
def check_bom_characters(repo_path: Path, result: ValidationResult, extensions: list[str] | None = None) -> list[Path]:
    """Scan for files with BOM characters."""
    if extensions is None:
        extensions = [".py", ".ts", ".js", ".tsx", ".jsx", ".json", ".toml", ".yaml", ".yml", ".md"]
    ...
    if bom_files:
        ...
        for f in bom_files[:5]:  # Show first 5
            result.add_info("encoding", f"  BOM in: {f.relative_to(repo_path)}", file_path=str(f))
        if len(bom_files) > 5:
...
def check_ollama_connectivity(config: dict[str, Any], result: ValidationResult) -> None:
    ...
    for profile_name, profile in config.get("embeddings", {}).get("profiles", {}).items():
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    result.add_warning("connectivity", f"Ollama at {url} returned status {resp.status}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Validate LLMC repository configuration")
    ...
    parser.add_argument("--no-models", action="store_true", help="Skip model availability checks")
    parser.add_argument("--fix-bom", action="store_true", help="Auto-fix BOM characters")
    parser.add_argument("-v", "--verbose", action="store_true", help="Show all info messages")
