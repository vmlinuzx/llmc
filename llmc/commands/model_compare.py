...
def compare_models(
    ...
    model_a: Optional[str] = typer.Option(
        None, "--model-a", "-a", help="Model A to compare (default: most common in baseline)"
    ),
    ...
    # Get model stats
    def get_model_stats(conn: sqlite3.Connection, model_filter: str | None = None) -> dict:
    ...
    if model_a and model_b:
        ...
        if stat_a and stat_b:
            ...
            for metric in ["avg_summary", "avg_tags", "avg_evidence", "avg_inputs", "avg_outputs"]:
                ...
                typer.echo(f"  {label:<12}: {val_a:>8.1f} → {val_b:>8.1f}  ({ratio:.2f}x)")
...
def metrics(
    ...
    if "tokens_per_second" not in cols:
        ...
        typer.echo("New enrichments will include metrics after updating to latest LLMC.")
        raise typer.Exit(0)
