...
class UserExecutor:
    ...
    def run(self):
        self.recorder.log_event("system", "user_executor", metadata={"msg": "Agent starting"})
        
        # 1. Understand Goal (Simulated)
        goal = self.scenario.goal
        self.recorder.log_event("thought", "user_executor", metadata={"thought": f"I need to achieve: {goal}"})
        ...
        if not queries:
            self.recorder.log_event("thought", "user_executor", metadata={"thought": "No explicit queries found in scenario. I am confused."})
            return
        ...
        self.recorder.log_event("system", "user_executor", metadata={"msg": "Agent finished"})
    ...
    def _step_search(self, query: str):
        ...
        if not tool:
            self.recorder.log_event("error", "user_executor", metadata={"msg": f"Tool {tool_name} not found"})
            return

        self.recorder.log_event("thought", "user_executor", metadata={"thought": f"I will search for '{query}'"})
        
        # Log Call
        self.recorder.log_event("tool_call", "user_executor", tool_name=tool_name, args={"query": query})
        ...
        try:
            ...
            summary = {
                "hit_count": len(items),
                "truncated": res_data.get("truncated", False) if isinstance(res_data, dict) else False,
                "item_ids": item_ids
            }
