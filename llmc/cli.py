...
try:
    ...
    print(
        "[bold red]ERROR:[/bold red] LLMC core modules not found. Ensure PYTHONPATH is set or you are in the correct directory."
    )
...
def monitor():
    ...
    with Live(layout, refresh_per_second=4, screen=True):
        while True:
            ...
            layout["help"].update(
                Panel(
                    Text(help_content, style="dim cyan"),
            ...
            layout["log"].update(
                Panel(
                    Text.from_markup(log_render),
                    title="[bold]Enrichment Log[/bold]",
                    border_style="green",
                )
            )
...
@app.command()
def route(
    test: str = typer.Option(..., "--test", help="Path to test for routing decision"),
    show_domain_decisions: bool = typer.Option(
        False, "--show-domain-decisions", help="Show reasoning"
    ),
):
