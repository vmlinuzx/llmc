...
class MedicalEmbeddingManager:
    ...
    def _embed_doc_longcontext(self, texts: str | list[str]) -> list[list[float]]:
    ...
    def _embed_section_ollama(self, texts: str | list[str]) -> list[list[float]]:
        if isinstance(texts, str):
            texts = [texts]
        ...
        try:
            ...
            try:
                ...
                metadata = EmbeddingMetadata(
                    provider="ollama", model="bge-m3", dimension=1024, profile="medical"
                )
                provider = OllamaEmbeddingProvider(
                    metadata=metadata, api_base="http://localhost:11434", timeout=60
                )
