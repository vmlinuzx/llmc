...
class TechDocsExtractor:
    ...
    def _render_node(self, node: dict[str, Any]) -> str:
        ...
        if type_ == "heading":
            ...
            text = "".join(
                self._render_node(child) for child in node.get("children", [])
            )
            return f"{ '#' * level} {text}\n\n"
    ...
    def extract(self, path: Path, content: str) -> Iterator[TechDocsSpan]:
        ...
        for node in ast:
            ...
            if node_type == "heading":
                ...
                if current_chunk_nodes:
                    ...
                    # Update line count
                    chunk_text = "".join(
                        self._render_node(n) for n in current_chunk_nodes
                    )
        ...
        if current_chunk_nodes:
            span = self._create_span(
                current_chunk_nodes, current_path_str, path, current_line, used_slugs
            )
            yield from self._maybe_split_span(span, used_slugs)

    def _maybe_split_span(
        self, span: TechDocsSpan, used_slugs: set
    ) -> Iterator[TechDocsSpan]:
        ...
        for para in paragraphs:
            ...
            if current_chars + para_chars > self.max_chunk_chars and current_chunk:
                ...
                yield TechDocsSpan(
                    ...
                    section_path=(
                        f"{span.section_path} (part {chunk_index})"
                        if span.section_path
                        else f"(part {chunk_index})"
                    ),
