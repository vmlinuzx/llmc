...
class ContextDetector:
    def __init__(self):
        ...
        self.historical_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.historical_patterns
        ]
        self.family_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.family_patterns
        ]
        self.hypothetical_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.hypothetical_patterns
        ]
    ...
    def detect_negation(self, text, entity_span):
        ...
        return self._check_patterns_in_context(
            self.negation_regex, pre_context, post_context
        )
    ...
    def detect_context(self, text, entity_span):
        ...
        return {
            "negated": self._check_patterns_in_context(
                self.negation_regex, pre_context, post_context
            ),
            "historical": self._check_patterns_in_context(
                self.historical_regex, pre_context, post_context
            ),
            "family": self._check_patterns_in_context(
                self.family_regex, pre_context, post_context
            ),
            "hypothetical": self._check_patterns_in_context(
                self.hypothetical_regex, pre_context, post_context
            ),
        }
