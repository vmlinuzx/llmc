...
class TechDocsEnrichment(BaseModel):
    ...
    # Core fields
    summary: str = Field(..., max_length=500, description="One sentence describing what this section explains")
    key_concepts: list[str] = Field(default_factory=list, description="Main concepts covered")
    parameters: list[ParameterDoc] = Field(default_factory=list, description="Documented parameters")
    
    # Boolean flags
    code_examples: bool = Field(False, description="Whether section contains code examples")
    ...
    # Evidence tracking
    evidence: list[dict[str, Any]] = Field(default_factory=list, description="Source evidence for extraction")
    
    # Truncation tracking
    truncated: bool = Field(False, description="True if any field was truncated during enrichment")
    truncated_fields: list[str] = Field(default_factory=list, description="Which fields were truncated")
    
    @field_validator('key_concepts', 'prerequisites', 'warnings', 'related_topics', mode='before')
    @classmethod
    ...
    def track_truncations(cls, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Check list lengths
        for field_name in ['key_concepts', 'prerequisites', 'warnings', 'related_topics']:
            ...
            for p in llm_output['parameters'][:MAX_LIST_ITEMS]:
                if isinstance(p, dict) and 'name' in p:
                    params.append(ParameterDoc(
                        name=p.get('name', '')[:100],
                        type=p.get('type'),
                        description=p.get('description', '')[:MAX_PARAM_DESCRIPTION]
                    ))
