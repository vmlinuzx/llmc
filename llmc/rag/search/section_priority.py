...
class SectionPriority:
    ...
    def boost_by_section(
        ...
        weights = (
            section_weights if section_weights is not None else cls.DEFAULT_WEIGHTS
        )
    ...
    def negation_aware_boost(cls, results: list[SearchResult]) -> list[SearchResult]:
        ...
        for result in results:
            ...
            # Check for negation
            is_negated = any(
                re.search(pattern, content) for pattern in negation_patterns
            )

            # Check for affirmation
            is_affirmed = any(
                re.search(pattern, content) for pattern in affirmation_patterns
            )
