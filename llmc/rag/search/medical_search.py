...
class MedicalSearchPipeline:
    ...
    def search(self, query: str, top_k: int = 10) -> List[SearchResult]:
        ...
        results = self._stage2_retrieve(query, candidate_docs=candidate_doc_ids, limit=top_k)
    ...
    def _stage2_retrieve(self, query: str, candidate_docs: List[str], limit: int) -> List[SearchResult]:
        # Implementation would call DB with filter
        return self.db.query_sections(query, doc_ids=candidate_docs, limit=limit)

    def _apply_tail_capture(self, current_results: List[SearchResult], candidate_docs: List[str]) -> List[SearchResult]:
        """
        ...
        existing_keys = {(r.doc_id, r.metadata.get("section_type")) for r in current_results}
