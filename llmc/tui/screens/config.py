...
class ConfigScreen(Screen):
    ...
    def _display_config(self) -> None:
        ...
        container.mount(Static("System & App Environment Variables", classes="config-section-title"))
        relevant_env_vars = {
        ...
        if relevant_env_vars:
            ...
            container.mount(Static("No specific LLMC/RAG related environment variables found.", classes="config-item"))
        ...
        try:
            ...
            container.mount(Static(f"[b]Index Exists[/b]: {index_path.exists()}", classes="config-item"))
        ...
        container.mount(Static(f"[b]Passage Prefix[/b]: '{embedding_passage_prefix()}'", classes="config-item"))
        container.mount(Static(f"[b]Query Prefix[/b]: '{embedding_query_prefix()}'", classes="config-item"))
        container.mount(Static(f"[b]Normalize[/b]: {embedding_normalize()}", classes="config-item"))
        container.mount(Static(f"[b]Device Pref[/b]: {embedding_device_preference()}", classes="config-item"))
        container.mount(Static(f"[b]Wait for GPU[/b]: {embedding_wait_for_gpu()}", classes="config-item"))
        container.mount(Static(f"[b]GPU Min Free MB[/b]: {embedding_gpu_min_free_mb()}", classes="config-item"))
        container.mount(Static(f"[b]GPU Max Retries[/b]: {embedding_gpu_max_retries()}", classes="config-item"))
        container.mount(Static(f"[b]GPU Retry Seconds[/b]: {embedding_gpu_retry_seconds()}", classes="config-item"))
        container.mount(Static("")) # Spacer
        ...
        container.mount(Static(f"[b]Python Version[/b]: {sys.version.splitlines()[0]}", classes="config-item"))
        ...
        container.mount(Static(f"[b]TUI CWD[/b]: {self.app.repo_root}", classes="config-item")) # This is the app's repo_root, might be different than cwd
        container.mount(Static("")) # Spacer
