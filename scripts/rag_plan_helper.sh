...
resolve_index_path() {
  ...
  local candidate
  candidate="$REPO_ROOT/.rag/index_v2.db"
  if [ -f "$candidate" ]; then
    printf '%s\n' "$candidate"
    return 0
  fi
  candidate="$REPO_ROOT/.rag/index.db"
  if [ -f "$candidate" ]; then
    printf '%s\n' "$candidate"
    return 0
  fi
...
RESULT="$("$PYTHON_BIN" "$SCRIPT_PATH" --repo "$REPO_ROOT" --limit "${RAG_PLAN_LIMIT:-5}" --min-score "${RAG_PLAN_MIN_SCORE:-0.4}" --min-confidence "${RAG_PLAN_MIN_CONFIDENCE:-0.6}" --no-log <<<"$QUERY" 2>&1 || echo "")"
