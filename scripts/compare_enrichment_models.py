...
def print_comparison(span_hash: str):
    ...
    print(textwrap.fill(old_data.get('summary', 'N/A'), width=78, initial_indent="  ", subsequent_indent="  "))
    print(f"\nTAGS: {old_data.get('tags', 'N/A')}")
    
    print(f"\n{'-'*35} QWEN 3:4B {'-'*34}")
    print(f"SUMMARY ({len(new_data.get('summary', ''))} chars):")
    print(textwrap.fill(new_data.get('summary', 'N/A'), width=78, initial_indent="  ", subsequent_indent="  "))
...
def main():
    ...
    print("-" * 75)
    print(f"{'Total Enrichments':<25} {old_stats.total_count:>15,} {new_stats.total_count:>15,} {new_stats.total_count - old_stats.total_count:>+15,}")
    print(f"{'Avg Summary Length':<25} {old_stats.avg_summary_len:>15.1f} {new_stats.avg_summary_len:>15.1f} {new_stats.avg_summary_len - old_stats.avg_summary_len:>+15.1f}")
    print(f"{'Avg Tags Length':<25} {old_stats.avg_tags_len:>15.1f} {new_stats.avg_tags_len:>15.1f} {new_stats.avg_tags_len - old_stats.avg_tags_len:>+15.1f}")
    print(f"{'Avg Evidence Length':<25} {old_stats.avg_evidence_len:>15.1f} {new_stats.avg_evidence_len:>15.1f} {new_stats.avg_evidence_len - old_stats.avg_evidence_len:>+15.1f}")
    print(f"{'Avg Inputs Length':<25} {old_stats.avg_inputs_len:>15.1f} {new_stats.avg_inputs_len:>15.1f} {new_stats.avg_inputs_len - old_stats.avg_inputs_len:>+15.1f}")
    print(f"{'Avg Outputs Length':<25} {old_stats.avg_outputs_len:>15.1f} {new_stats.avg_outputs_len:>15.1f} {new_stats.avg_outputs_len - old_stats.avg_outputs_len:>+15.1f}")
    print("-" * 75)
    print(f"{'Empty Summaries':<25} {old_stats.empty_summary_count:>15,} {new_stats.empty_summary_count:>15,}")
    print(f"{'Empty Tags':<25} {old_stats.empty_tags_count:>15,} {new_stats.empty_tags_count:>15,}")
    print(f"{'Empty Evidence':<25} {old_stats.empty_evidence_count:>15,} {new_stats.empty_evidence_count:>15,}")
    ...
    for label, stats in [("Qwen 2.5:7B", old_stats), ("Qwen 3:4B", new_stats)]:
        ...
        if lens:
            ...
            print(f"\n{label}:")
            print(f"  Min: {min(lens):>4}  P25: {p25:>4.0f}  Median: {p50:>4.0f}  P75: {p75:>4.0f}  Max: {max(lens):>4}")
    
    # Verbosity ratio
    verbosity_ratio = new_stats.avg_summary_len / old_stats.avg_summary_len if old_stats.avg_summary_len else 0
