...
class AgentSimulator:
    ...
    def run_file_edit_workload(self, duration_sec: int):
        ...
        test_files = [
            self.work_dir / f"agent_{self.agent_id}_file_{i}.py" for i in range(3)
        ]
        ...
        while time.time() < end_time:
            try:
                ...
                result = get_maasl().call_with_stomp_guard(
                    op=db_write,
                    resources=[
                        ResourceDescriptor("CRIT_DB", str(self.work_dir / "test.db"))
                    ],
                    intent="db_write",
                    mode="interactive",
                    agent_id=str(self.agent_id),
...
def analyze_results(results: list[dict]):
    ...
    print(
        f"  Success rate:         {(total_ops / (total_ops + total_errors) * 100):.1f}%"
    ...
    for r in results:
        print(
            f"  Agent {r['agent_id']}: {r['operations']} ops, "
...
def main():
    ...
    parser.add_argument(
        "--agents", type=int, default=5, help="Number of concurrent agents (default: 5)"
    )
    parser.add_argument(
        "--duration",
        type=int,
    ...
    parser.add_argument(
        "--lint", action="store_true", help="Run ruff lint check on generated files"
    )
