        ...
        with patch("llmc_agent.agent.OllamaBackend") as MockOllamaBackend:
            ...
            error_msg = next(
                (
                    ...
                    and "not available in current tier" in str(msg.get("content"))
                ),
                None,
            )
            ...
            ), "Expected error message about unavailable tool not found in messages."
            assert (
                "WALK" in error_msg["content"]
            ), "Error message should mention the current tier (WALK)."
