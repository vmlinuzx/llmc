...
"""
    ...
    assert config.tools.run_cmd_blacklist == [], "Blacklist should be empty as it wasn't specified"
    
    # Check that the config object doesn't even have the allowlist field (confirming the gap)
    assert not hasattr(config.tools, "run_cmd_allowlist"), "Config model is missing 'run_cmd_allowlist'"
    ...
    with patch("llmc_mcp.isolation.require_isolation") as mock_iso:
        ...
        result = run_cmd("whoami", cwd=tmp_path, blacklist=config.tools.run_cmd_blacklist)
        ...
        assert result.success is False, f"Security Breach: 'whoami' was allowed despite not being in allowlist! Stdout: {result.stdout.strip()}"
        assert result.error and "Security" in result.error, f"Error should be security related, got: {result.error}"
