...
class TestRAGArgInjection(unittest.TestCase):
    ...
    def test_search_arg_injection(self):
        ...
        with patch('llmc_agent.backends.llmc.LLMCBackend._find_repo_root', return_value=Path("/mock/repo")):
                ...
                for call_args in mock_run.call_args_list:
                    ...
                    if query in cmd_list:
                        ...
                        if query_index > 0:
                            ...
                            self.assertEqual(prev_arg, "--", 
                                             f"Query '{query}' was not preceded by '--'. Command was: {cmd_list}")
                        else:
                             self.fail(f"Query '{query}' was the first argument? Command: {cmd_list}")
