...
from llmc_agent.session import SessionManager
...
def test_session_path_traversal(temp_session_env):
    ...
    assert (
        loaded_session.id == expected_session_data["id"]
    ), "Loaded session ID does not match expected."
    assert (
        loaded_session.messages[0].content
        == expected_session_data["messages"][0]["content"]
    ...
    assert (
        actual_path_attempted.resolve() == expected_full_path.resolve()
