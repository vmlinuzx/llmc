...
class TestChainOperations:
    ...
    def test_deep_copy_verification(self):
        ...
        assert (
            source_chain["parameters"]["temp"] == 0.7
    ...
    def test_tier_sorting(self):
        ...
        ), f"Tier sorting incorrect. Got {ordered_names}, expected {expected_order}"
    ...
    def test_safe_deletion_unique_in_route(self):
        ...
        initial_config = {
            "enrichment": {
                "chain": [{"name": "c1", "chain": "g1", "enabled": True}],
                "routes": {"summary": "g1"},
            }
        }
        ...
        assert (
            can_delete is False
        ), "Should block deletion when chain is the only one in a route group"
    ...
    def test_safe_deletion_unused(self):
        ...
        assert (
            can_delete is True
        ), "Should allow deletion when chain is not used by any route"
