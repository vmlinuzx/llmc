...
from llmc_agent.backends.base import GenerateResponse  # Import GenerateResponse
...
class TestAgentContextOverflow:
    ...
    async def test_unbounded_tool_output_context_overflow(
        ...
        second_call_args, _ = (
            mock_ollama_backend_instance.generate_with_tools.call_args_list[1]
        )
        messages_sent_to_llm = second_call_args[
            0
        ].messages  # Access messages attribute of GenerateRequest
        ...
        ), "Tool output message not found in the second LLM call."

        # Assert that the content of the tool output message is the full, untruncated string
        assert tool_output_message["content"] == json.dumps(
            VERY_LARGE_STRING
        )  # Tool output is json dumped
