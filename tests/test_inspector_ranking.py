...
def mock_graph():
    ...
    graph.entities.append(Entity(
        id="type:module.PublicClass", kind="class", path="module.py:10-50",
        file_path="module.py", start_line=10, end_line=50
    ))
    # 2. Private Helper Function (Less Important)
    graph.entities.append(Entity(
        id="sym:module._helper", kind="function", path="module.py:60-70",
        file_path="module.py", start_line=60, end_line=70
    ))
    # 3. Public Function (Important)
    graph.entities.append(Entity(
        id="sym:module.public_func", kind="function", path="module.py:80-100",
        file_path="module.py", start_line=80, end_line=100
    ))
    # 4. Small Variable (Least Important)
    graph.entities.append(Entity(
        id="sym:module.CONST", kind="variable", path="module.py:5-5",
        file_path="module.py", start_line=5, end_line=5
    ))
    
    # Add some relations to boost importance
    # PublicClass is used by something
    graph.relations.append(Relation(src="sym:other.func", edge="uses", dst="type:module.PublicClass"))
