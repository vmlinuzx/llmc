...
class TestMedicalRAGIntegration:
    ...
    def test_section_detection_accuracy(self, sample_notes):
        ...
        print(
            f"Section detection accuracy: {accuracy:.3f} ({detected_sections}/{total_sections})"
        )
        assert (
            accuracy >= 0.95
        ), f"Section detection accuracy {accuracy:.3f} below target 0.95"
    ...
    def test_full_search_pipeline(self, repo_root, sample_notes):
        ...
        for query in test_queries:
            try:
                results = search_spans(
                    query=query, limit=5, repo_root=repo_root, debug=False
                )
        ...
        print(
            f"Search pipeline success rate: {success_rate:.3f} ({successful_searches}/{len(test_queries)})"
        )
        assert (
            success_rate >= 0.8
        ), f"Search pipeline success rate {success_rate:.3f} below target 0.8"
