...
"""
        ...
        # Check for types
        assert any(
            "Request" in eid for eid in entity_ids
        ), "Request interface not found"
        assert any(
            "Response" in eid for eid in entity_ids
        ), "Response interface not found"
        assert any(
            "RouteHandler" in eid for eid in entity_ids
        ), "RouteHandler type not found"
        ...
        assert any(
            "logRequest" in eid for eid in entity_ids
        ), "Handler.logRequest not found"

        # Check for functions
        assert any(
            "createRouter" in eid for eid in entity_ids
        ), "createRouter function not found"

        # ===== Verify entity details =====
        handler_class = next(
            e for e in graph.entities if "Handler" in e.id and e.kind == "class"
        )
        ...
        route_calls = [
            r for r in graph.relations if "route" in r.src and r.edge == "calls"
        ]
        assert (
            len(route_calls) >= 2
        ), f"Expected at least 2 calls from route, got {len(route_calls)}"
        ...
        print(
            f"✅ Successfully extracted {len(graph.entities)} entities and {len(graph.relations)} relations"
        )
        print(f"   Entities: {', '.join(e.id[:40] for e in graph.entities[:5])}...")
        print(
            f"   Relations: {len([r for r in graph.relations if r.edge == 'calls'])} calls, "
