...
if not db_path.exists():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS telemetry_events (timestamp TEXT, cmd TEXT, mode TEXT, output_size INTEGER, latency_ms REAL, agent_id TEXT)")
    conn.execute("INSERT INTO telemetry_events VALUES (datetime('now'), 'ls', 'passthrough', 100, 10.0, 'test')")
    conn.commit()
    conn.close()
