...
def test_te_cli_command_injection():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert kwargs.get("shell") is True, "CRITICAL: TE CLI must not use shell=True for pass-through"
...
def test_te_run_isolation_bypass():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        with patch("llmc_mcp.isolation.require_isolation", side_effect=RuntimeError("Isolation Required!")) as mock_iso:
            try:
                te_run(["run", "echo", "hello"])
            except RuntimeError:
                pytest.fail("te_run called require_isolation() - it is secure (unexpected).")
            
            # If we get here, no exception was raised
            assert not mock_iso.called, "te_run called require_isolation (unexpectedly secure)"
