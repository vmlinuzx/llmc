...
def test_run_cmd_allows_anything_if_isolated(tmp_path):
    ...
    with patch("llmc_mcp.isolation.require_isolation") as mock_iso:
        ...
        if "; echo B" in result_chain.stdout or "A; echo B" in result_chain.stdout:
            ...
            "B" == result_chain.stdout.strip()
        ):  # If it just printed B, chaining worked!
        ...
        if result_py.success and "2" in result_py.stdout:
            print(
                "[+] Verified: We can run arbitrary code via interpreters (python/perl/etc) because they are not blacklisted."
            )
            pytest.fail(
                "CRITICAL: Empty blacklist allows executing interpreters (python, etc)."
            )
