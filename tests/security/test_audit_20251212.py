            ...
            if cmd_list[0] == "rg":
                ...
                if payload in cmd_list:
                    try:
                        ...
                        has_separator = (
                            "--" in cmd_list and cmd_list.index("--") < payload_idx
                        )

                        if not has_separator:
                            pytest.fail(
                                f"VULNERABILITY CONFIRMED: Argument '{payload}' treated as flag. Command: {cmd_list}"
                            )
                    except ValueError:
                        pass
...
"""
        start = time.time()
        result = execute_code(
            code=code, tool_caller=lambda n, a: None, timeout=1  # Request 1s timeout
        )
        end = time.time()
        ...
        if duration >= 2.0:
            pytest.fail(
                f"VULNERABILITY CONFIRMED: execute_code failed to enforce timeout. Took {duration:.2f}s > 1s"
            )
...
def test_poc_code_exec_environ_leak():
    ...
    with patch("llmc_mcp.isolation.require_isolation"):
        ...
        execute_code(code=code, tool_caller=lambda n, a: None)

        # Check if environment leaked to host
        if os.environ.get(key) == val:
            del os.environ[key]  # cleanup
            pytest.fail(
                "VULNERABILITY CONFIRMED: execute_code modified host process environment variables."
            )
