...
def test_shell_backend_happy_path(echo_script, tmp_path):
    ...
    backend = ShellDocgenBackend(
        script=Path(sys.executable), args=[str(echo_script)], timeout_seconds=5
    )
...
def test_shell_backend_garbage_output(garbage_script, tmp_path):
    backend = ShellDocgenBackend(
        script=Path(sys.executable), args=[str(garbage_script)], timeout_seconds=5
    )
...
"""
    ...
    backend = ShellDocgenBackend(
        script=Path(sys.executable), args=[str(script_path)], timeout_seconds=5
    )
