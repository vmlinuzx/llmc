...
"""
    ...
    try:
        ...
        assert any(
            "initRouter" in eid for eid in entity_ids
        ), "initRouter function not found"

        # Check entity kinds
        router_entity = next(
            e for e in entities if "Router" in e.id and e.kind == "class"
        )
...
"""
    ...
    try:
        ...
        assert any(
            "Internal" in eid for eid in entity_ids
        ), "Internal interface not found"
...
"""
    ...
    try:
        ...
        router_extends = next(
            r for r in extends_relations if "Router" in r.src and "BaseRouter" in r.dst
        )
...
"""
    ...
    try:
        ...
        assert any(
            "createHandler" in eid for eid in entity_ids
        ), "createHandler function not found"
