...
""")
    ...
    row_code = db.conn.execute(
        "SELECT slice_type, slice_language FROM spans WHERE file_id = (SELECT id FROM files WHERE path='code.py')"
    ).fetchone()
    assert row_code["slice_type"] == "code"
    assert row_code["slice_language"] == "python"

    row_doc = db.conn.execute(
        "SELECT slice_type FROM spans WHERE file_id = (SELECT id FROM files WHERE path='doc.md')"
    ).fetchone()
    ...
    enrich_code = db.conn.execute(
        "SELECT content_type, content_language FROM enrichments WHERE span_hash = ?",
        (code_span_hash,),
    ).fetchone()
...
""")
    ...
    with caplog.at_level(logging.WARNING):
        ...
        assert route == "docs"
        assert (
            "Missing 'routing.slice_type_to_route' entry for slice_type='weird_type'. Defaulting to route_name='docs'."
            in caplog.text
        )
...
""")
    ...
    with caplog.at_level(logging.WARNING):
        ...
        assert (
            "Missing 'embeddings.routes.code' for query. Falling back to 'docs' route."
            in caplog.text
        )
...
""")
    ...
    with caplog.at_level(logging.WARNING):
        ...
        assert index == "emb_docs"
        assert (
            "Route 'code' for query has incomplete definition (profile: code_jina, index: None). Falling back to 'docs' route."
            in caplog.text
        )
        # Ensure that the ConfigWarningFilter works
        profile, index = resolve_route("code", "query", repo_root)
...
""")
    ...
    assert (
        "Config Error: Route 'code' for ingest refers to missing profile 'missing_profile'"
...
""")
    ...
    (repo_root / "code.py").write_text(
        "def foo(): pass"
    )  # Will fall back due to missing_code_route
    (repo_root / "doc.md").write_text("# Doc")  # Should go to docs
    (repo_root / "config.weird").write_text(
        "key=value"
    )  # Will fall back due to non_existent_profile_route
    ...
    assert (
        "Missing 'embeddings.routes.missing_code_route' for ingest. Falling back to 'docs' route."
        in caplog.text
    )
...
""")
    ...
    db = Database(db_path)
    db.conn.execute(
        "CREATE TABLE IF NOT EXISTS emb_docs (span_hash TEXT PRIMARY KEY, vec BLOB, route_name TEXT, profile_name TEXT)"
    )
    ...
    db.conn.execute(
        "INSERT INTO emb_docs (span_hash, vec, route_name, profile_name) VALUES (?, ?, ?, ?)",
        ("hash_doc", dummy_vec, "docs", "default_docs"),
    )
    db.conn.commit()
    db.close()
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert (
            "Query routing classification: route='code' confidence=0.90 reasons=['code_heuristic_match']"
            in caplog.text
        )
        assert (
