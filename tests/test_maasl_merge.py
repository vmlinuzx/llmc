...
from llmc.rag.graph_store import GraphStore
...
def test_add_edges(merge_engine, empty_graph):
    ...
    result = merge_engine.apply_patch(
        patch2, empty_graph, agent_id="agent1", session_id="s1"
    )
...
def test_lww_node_conflict(merge_engine, empty_graph):
    ...
    patch1 = GraphPatch(
        nodes_to_add=[{"id": "func1", "kind": "function", "summary": "version 1"}]
    )
    merge_engine.apply_patch(patch1, empty_graph, agent_id="agent1", session_id="s1")

    # Agent 2 adds same node with summary "v2" (LWW - should overwrite)
    patch2 = GraphPatch(
        nodes_to_add=[{"id": "func1", "kind": "function", "summary": "version 2"}]
    )
    result = merge_engine.apply_patch(
        patch2, empty_graph, agent_id="agent2", session_id="s2"
    )
...
def test_update_properties(merge_engine, empty_graph):
    ...
    # Add node
    patch1 = GraphPatch(nodes_to_add=[{"id": "func1", "kind": "function"}])
    merge_engine.apply_patch(patch1, empty_graph, agent_id="agent1", session_id="s1")
...
def test_clear_properties(merge_engine, empty_graph):
    ...
    # Clear property
    patch2 = GraphPatch(properties_to_clear={"func1": ["temp_data"]})
    result = merge_engine.apply_patch(
        patch2, empty_graph, agent_id="agent1", session_id="s1"
    )
...
def test_concurrent_graph_updates(merge_engine, empty_graph):
    ...
    def update_agent(agent_id: str, node_id: str):
        ...
        patch = GraphPatch(
            nodes_to_add=[{"id": node_id, "kind": "function", "agent": agent_id}]
        )
        result = merge_engine.apply_patch(
            patch,
            empty_graph,
...
def test_deterministic_ordering(merge_engine, empty_graph):
    ...
    result1 = merge_engine.apply_patch(
        patch, empty_graph, agent_id="agent1", session_id="s1"
    )
