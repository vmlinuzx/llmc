...
def test_routing_tier_arbitrary_strings():
    ...
    spec_8b = EnrichmentBackendSpec(
        name="test-8b", routing_tier="8b", provider="ollama"
    )
    spec_weird = EnrichmentBackendSpec(
        name="test-weird", routing_tier="super-mega-model-v1", provider="ollama"
    )
    spec_empty = EnrichmentBackendSpec(
        name="test-empty", routing_tier="", provider="ollama"
    )
    spec_spaces = EnrichmentBackendSpec(
        name="test-spaces", routing_tier="  spaced  ", provider="ollama"
    )
...
def test_routing_tier_none_fallback():
    ...
    spec_none = EnrichmentBackendSpec(
        name="test-none", routing_tier=None, provider="ollama"
    )
    spec_7b = EnrichmentBackendSpec(
        name="test-7b", routing_tier="7b", provider="ollama"
    )
...
def test_parse_backend_spec_invalid_provider():
    ...
    with pytest.raises(EnrichmentConfigError):
        _parse_backend_spec(
            {"name": "bad", "provider": "unsupported"}, default_chain="default"
        )
