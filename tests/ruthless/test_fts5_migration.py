...
def test_fts5_migration(tmp_path):
    ...
    db.conn.execute(
        "INSERT INTO files (path, lang, file_hash, size, mtime) VALUES ('f1', 'py', 'h1', 10, 0)"
    )
    file_id = db.conn.execute("SELECT id FROM files").fetchone()[0]
    db.conn.execute(
        f"INSERT INTO spans (file_id, symbol, kind, start_line, end_line, byte_start, byte_end, span_hash) VALUES ({file_id}, 'sym1', 'func', 1, 1, 0, 10, 'sh1')"
    )
    db.conn.execute(
        "INSERT INTO enrichments (span_hash, summary, model) VALUES ('sh1', 'This is a model for the system', 'm1')"
    )
    ...
    cursor = db.conn.execute(
        "SELECT * FROM enrichments_fts WHERE enrichments_fts MATCH 'the'"
    )
    results = cursor.fetchall()
    assert len(results) > 0, "Should find 'the' if stopwords are disabled"

    cursor = db.conn.execute(
        "SELECT * FROM enrichments_fts WHERE enrichments_fts MATCH 'model'"
    )
    results = cursor.fetchall()
    ...
    cursor = db.conn.execute(
        "SELECT sql FROM sqlite_master WHERE name='enrichments_fts'"
    )
