...
class TestConfigValidation:
    ...
    def test_validate_missing_toml_file(self, tmp_path):
        ...
        assert any("llmc.toml" in str(issue) or "not found" in str(issue).lower() 
...
class TestBOMDetection:
    ...
    def test_detect_bom_in_file(self, tmp_path):
        ...
        from llmc.commands.repo_validator import check_bom_characters, ValidationResult
    ...
    def test_fix_bom_in_file(self, tmp_path):
        ...
        from llmc.commands.repo_validator import fix_bom_characters, ValidationResult
...
class TestOllamaConnectivity:
    ...
    def test_ollama_reachable(self, mock_urlopen, tmp_path):
        ...
        from llmc.commands.repo_validator import check_ollama_connectivity, ValidationResult
        ...
        result.config = {
            "enrichment": {
                "chain": [{"provider": "ollama", "url": "http://localhost:11434", "enabled": True}]
            }
        }
    ...
    def test_ollama_unreachable(self, mock_urlopen, tmp_path):
        ...
        assert result.error_count > 0 or any("connect" in str(i).lower() for i in result.issues)
...
""")
        ...
        parsing_errors = [i for i in result.issues if "parse" in str(i).lower() and i.severity == "error"]
...
class TestCLIIntegration:
    ...
    def test_llmc_repo_validate_command_exists(self):
        ...
        assert result.returncode == 0 or "validate" in result.stdout or "validate" in result.stderr
