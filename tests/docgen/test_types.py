...
def test_docgen_result_valid_status():
    ...
    # Test "noop"
    result = DocgenResult(
        status="noop", sha256="abc123", output_markdown=None, reason="SHA unchanged"
    )
...
def test_docgen_result_invalid_status():
    ...
    with pytest.raises(ValueError, match="Invalid status 'invalid'"):
        DocgenResult(status="invalid", sha256="abc123", output_markdown=None)

    with pytest.raises(ValueError, match="Invalid status 'GENERATED'"):
        DocgenResult(
            status="GENERATED", sha256="abc123", output_markdown=None  # Wrong case
        )


def test_docgen_result_default_reason():
    """Test DocgenResult with default reason=None."""
    result = DocgenResult(status="noop", sha256="abc123", output_markdown=None)
