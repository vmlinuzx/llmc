...
def setup_mock_db(temp_db_path):
    ...
    # Insert sample data
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    # Routing events
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:10:00Z', '[routing_ingest_slice] slice_type=code, route_name=code, profile_name=code_jina', 'routing_ingest_slice', 0, 0, 0, 0, 10.0, None, ''))
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:11:00Z', '[routing_ingest_slice] slice_type=docs, route_name=docs, profile_name=docs_text', 'routing_ingest_slice', 0, 0, 0, 0, 12.0, None, ''))
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:12:00Z', '[routing_ingest_slice] slice_type=code, route_name=code, profile_name=code_jina', 'routing_ingest_slice', 0, 0, 0, 0, 11.0, None, ''))
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:13:00Z', '[routing_query_classify] route_name=code, confidence=0.8', 'routing_query_classify', 0, 0, 0, 0, 5.0, None, ''))
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:14:00Z', '[routing_query_classify] route_name=docs, confidence=0.9', 'routing_query_classify', 0, 0, 0, 0, 6.0, None, ''))
    cursor.execute("INSERT INTO telemetry_events VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   ('2025-12-01T10:15:00Z', '[routing_fallback] type=missing_slice_type_mapping, slice_type=weird_type, fallback_to=docs', 'routing_fallback', 0, 0, 0, 0, 3.0, None, ''))
...
class TestTeStatsOutput:
    ...
    def test_stats_output_format(self, mock_find_repo_root, mock_stdout, temp_db_path, setup_mock_db):
        """
...
└───────────────────────────────────────────────────────┘"""
        ...
        for i, (actual_line, expected_line) in enumerate(zip(actual_output_lines, expected_output_lines)):
            assert actual_line == expected_line, f"Line {i+1} differs: Actual='{actual_line}', Expected='{expected_line}'"
