...
def test_single_agent_write(db_manager):
    ...
    with db_manager.write_transaction(agent_id="agent1", session_id="session1") as conn:
        conn.execute(
            "INSERT INTO test_data (agent_id, value) VALUES (?, ?)", ("agent1", 42)
        )
...
def test_read_transaction(db_manager):
    ...
    # Insert test data
    db_manager.db_conn.execute(
        "INSERT INTO test_data (agent_id, value) VALUES (?, ?)", ("test", 1)
    )
...
def test_concurrent_writes_different_dbs():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def write_agent(agent_id: str, db_path: Path, db_name: str):
            ...
            try:
                with mgr.write_transaction(
                    agent_id=agent_id, session_id=f"session_{agent_id}"
                ) as c:
...
def test_concurrent_writes_same_db_contention(db_manager):
    ...
    def write_agent(agent_id: str):
        ...
        try:
            ...
            with db_manager.write_transaction(
                ...
                conn.execute(
                    "INSERT INTO test_data (agent_id, value) VALUES (?, ?)",
                    (agent_id, 1),
                )
    ...
    # Launch 3 agents
    threads = [
        threading.Thread(target=write_agent, args=(f"agent{i}",)) for i in range(1, 4)
    ]
    ...
    assert len(rows) == len(
        successes
    ), f"Data corruption: {len(rows)} rows != {len(successes)} successes"
...
def test_transaction_rollback_on_error(db_manager):
    ...
    try:
        with db_manager.write_transaction(
            agent_id="agent1", session_id="session1"
        ) as conn:
            conn.execute(
                "INSERT INTO test_data (agent_id, value) VALUES (?,  ?)", ("agent1", 1)
            )
            # Trigger error
