...
class TestFsProtected(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_maasl_patcher = patch(
            "llmc_mcp.tools.fs_protected.get_maasl", return_value=self.mock_maasl
        )
        ...
        self.mock_write_patcher = patch(
            "llmc_mcp.tools.fs_protected._write_file_unprotected"
        )
        self.mock_write = self.mock_write_patcher.start()

        self.mock_edit_patcher = patch(
            "llmc_mcp.tools.fs_protected._edit_block_unprotected"
        )
        self.mock_edit = self.mock_edit_patcher.start()

        self.mock_move_patcher = patch(
            "llmc_mcp.tools.fs_protected._move_file_unprotected"
        )
        self.mock_move = self.mock_move_patcher.start()

        self.mock_delete_patcher = patch(
            "llmc_mcp.tools.fs_protected._delete_file_unprotected"
        )
    ...
    def test_write_file_protected_success(self):
        # Setup
        expected_result = FsResult(
            success=True, data={"bytes_written": 100, "sha256": "hash"}, meta={}
        )
    ...
    def test_write_file_protected_path_validation_fail(self):
        ...
        # Action
        result = write_file_protected(
            path="/unsafe/file.txt", allowed_roots=self.allowed_roots, content="content"
        )
    ...
    def test_move_file_protected_locks_both_paths(self):
        # Setup
        expected_result = FsResult(
            success=True, data={"source": "src", "dest": "dst"}, meta={}
        )
        self.mock_maasl.call_with_stomp_guard.return_value = expected_result
    ...
    def test_delete_file_protected_success(self):
        # Setup
        expected_result = FsResult(
            success=True, data={"deleted": "/tmp/safe/file.txt"}, meta={}
        )
        self.mock_maasl.call_with_stomp_guard.return_value = expected_result

        # Action
        result = delete_file_protected(
            path="/tmp/safe/file.txt", allowed_roots=self.allowed_roots, recursive=True
        )
