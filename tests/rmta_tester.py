...
async def main():
    ...
    try:
        config = load_config()
        print(
            f"Config loaded. Mode: {'Code Execution' if config.code_execution.enabled else 'Classic'}"
        )
    ...
    if config.code_execution.enabled and "execute_code" in tool_names:
        ...
        test_cases = [
            (
                "RAG Search",
                "from stubs import rag_search; print(rag_search(query='config'))",
            ),
            (
                "Read File",
                "from stubs import read_file; print(read_file(path='README.md'))",
            ),
            (
                "Proc List",
                "from stubs import linux_proc_list; print(linux_proc_list(max_results=2))",
            ),
            (
                "System Snapshot",
                "from stubs import linux_sys_snapshot; print(linux_sys_snapshot())",
            ),
            ("Metric Check", "from stubs import get_metrics; print(get_metrics())"),
            (
                "FS Write",
                "from stubs import linux_fs_write; print(linux_fs_write(path='tests/rmta_artifact.txt', content='RMTA was here'))",
            ),
            (
                "Run Cmd LS",
                "from stubs import run_cmd; print(run_cmd(command='ls -la tests/'))",
            ),
            (
                "Run Cmd Forbidden",
                "from stubs import run_cmd; print(run_cmd(command='rm -rf /'))",
            ),
            # Intentional Fail
