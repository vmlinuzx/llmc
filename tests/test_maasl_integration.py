...
def test_concurrent_writes_different_files(temp_dir, allowed_roots):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(write_agent, f"agent{i}") for i in range(1, 4)]
...
def test_concurrent_writes_same_file_contention(test_file, allowed_roots):
    ...
    assert (
        len(failures) >= 1
    ), f"Expected at least 1 failure (contention), got {len(failures)}"
...
def test_concurrent_edits_same_file(test_file, allowed_roots):
    ...
    threads = [
        threading.Thread(target=edit_agent, args=(f"agent{i}",)) for i in range(1, 3)
    ]
...
def test_move_file_protection(temp_dir, allowed_roots):
    ...
    threads = [
        threading.Thread(target=move_agent, args=(f"agent{i}",)) for i in range(1, 3)
    ]
...
def test_delete_file_protection(temp_dir, allowed_roots):
    ...
    threads = [
        threading.Thread(target=delete_agent, args=(f"agent{i}",)) for i in range(1, 3)
    ]
