...
def mock_dependencies():
    with patch("tools.rag.search.find_repo_root") as mock_root, \
        ...
        def iter_embeddings_side_effect(table_name):
            if table_name == "index_a":
                return [{
                    "vec": vec, "span_hash": "hash_a", "file_path": "file_a.py", 
                    "symbol": "sym_a", "kind": "func", "start_line": 1, "end_line": 10, "summary": "sum_a"
                }]
            elif table_name == "index_b":
                return [{
                    "vec": vec, "span_hash": "hash_b", "file_path": "file_b.py", 
                    "symbol": "sym_b", "kind": "func", "start_line": 20, "end_line": 30, "summary": "sum_b"
                }]
            return []
...
def test_search_multi_route_fanout(mock_dependencies):
    ...
    def resolve_side_effect(route, op, repo):
        if route == "route_a": return ("profile_a", "index_a")
        if route == "route_b": return ("profile_b", "index_b")
