            ...
            for f in os.listdir(stubs_dir):
                if f.endswith(".py") and f != "__init__.py":
                    stubs.append(
                        {
                            "name": f[:-3],
                            "description": "Stubbed tool (via execute_code)",
                            "is_stub": True,
                        }
                    )
        ...
        for tool in all_tools:
            ...
            if is_stub:
                ...
                if "error" in call_res:
                    ...
                    result_entry["notes"] = (
                        f"execute_code failed: {call_res['error'].get('message')}"
                    )
                    ...
                    if is_error:  # Python runtime error
                        result_entry["status"] = "❌ Broken"
                        result_entry["notes"] = (
                            f"Python error: {text_out.strip()[:100]}"
                        )
                        report["summary"]["broken"] += 1
                    elif "ImportError" in text_out:
                        result_entry["status"] = "❌ Broken"
                        result_entry["notes"] = (
                            f"Import failed: {text_out.strip()[:100]}"
                        )
                        report["summary"]["broken"] += 1
                    else:
                        result_entry["status"] = "✅ Working"
                        result_entry["notes"] = (
                            f"Stub execution success. Output: {text_out.strip()[:50]}..."
                        )
            ...
            try:
                ...
                if "error" in call_res:
                    ...
                    if is_error:
                        result_entry["status"] = "❌ Broken"
                        result_entry["notes"] = (
                            f"Tool returned isError=True. Content: {str(content)[:100]}..."
                        )
                        report["summary"]["broken"] += 1
                    else:
                        result_entry["status"] = "✅ Working"
                        result_entry["notes"] = (
                            f"Success. Duration: {duration:.2f}s. Content len: {len(str(content))}"
                        )
                        report["summary"]["working"] += 1
