...
for res in "${LOCKS[@]}"; do
  tries=0
  while true; do
    out=$(python3 scripts/llmc_lock.py acquire --resource "$res" --task-id "$ID" --ttl "$LEASE_TTL" --started-at "$STARTED_AT" || true)
...
GATE_CMD=(scripts/integration_gate.sh "${WT_DIR}")
...
for res in "${LOCKS[@]}"; do
  python3 scripts/llmc_lock.py release --resource "$res" --task-id "$ID" >/dev/null || true
done
