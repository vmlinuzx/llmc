...
- **P0: Ruthless Testing Fallout – Criticality Path (2025-11-20)**  
  Source of truth: `tests/reports/ruthless_testing_report_v3.md`. Prioritize in this order so we stop the bleeding before chasing polish.
  1. **Registry validation + path blocking** – `tests/test_ruthless_edge_cases.py` expects hard failures for malformed YAML, missing mandatory fields, binary blobs, and path expansion exploits. `tools/rag_daemon/registry.py` currently lets everything through, which is a security regression. Restore schema validation, sanitize paths (reuse `safe_resolve` helpers), and assert exceptions are propagated.
  2. **Wrapper script timeouts** – `tests/test_wrapper_scripts.py` now surfaces real `subprocess.TimeoutExpired` failures for `tools/claude_minimax_rag_wrapper.sh` and friends. Investigate hanging points (likely waiting on deps or logs), add watchdog logging, and ensure wrappers exit cleanly under 10s or provide progress pings.
  3. **Worker pool timing/state drift** – `tests/test_worker_pool_comprehensive.py` catches inaccurate wall-clock math (<60s windows), empty tracking sets, and bad failure counts. Audit `tools/rag_daemon/registry.py` + worker pool bookkeeping: use monotonic timers, enforce thread-safe state mutation, and add regression metrics.
  4. **Filesystem & symlink containment** – `tests/test_safecopy_move_policy.py::test_copy_and_move_inside_base` and `tests/test_symlink_escape_strict.py::test_symlink_escape_blocked` both raise `FileExistsError` instead of enforcing copy/move + symlink policy. Align `tools/rag/utils.py` + `tools/rag/workspace.py` with the centralized `safe_resolve` behavior and add guard rails so outside-the-base writes error before touching disk.
  5. **SQLite reserved keyword + DB bootstrapping** – `tools/rag/analytics.py:135` still runs `SELECT unique(...)` (16 failures) and `tools/rag/database.py:81` opens non-existent files (7 failures). Quote identifiers or swap to `DISTINCT`, eagerly create the DB (including directories), and add smoke tests that open/close a fresh workspace DB successfully.
  6. **Router + benchmark attribute drift** – `tests/test_rag_router.py` (8 fails) and `tools/rag/benchmark.py` (missing attribute errors) show the router returning `None`, wrong tiers, and cost estimates < expected. Reconcile routing heuristics with current config defaults and add explicit data classes for benchmark configs so attribute access is deterministic.
  7. **Scheduler eligibility regressions** – `tests/test_scheduler_eligibility_comprehensive.py` fails three scenarios where job gating should be strict. Add explicit acceptance criteria for fairness/priority ordering and assert the tests' failure modes first (xfail or targeted fix).
  8. **Path traversal tests now expect enforcement** – previously demo tests simply logged vulnerabilities; update the suites (or code) so that `test_export_path_safety.py`, `test_export_force_guard.py`, and related cases fail only when the guard is missing. Either harden the guards now or mark tests `xfail` with links to the implementation ticket.
