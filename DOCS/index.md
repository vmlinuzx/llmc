# LLMC Documentation

**The Large Language Model Compressor** — Stop burning money on LLM tokens.

Welcome to the official documentation for LLMC. Whether you are installing LLMC for the first time, integrating it into your workflow, or contributing to the source code, this guide covers everything you need.

---

## 🚀 Start Here

New to LLMC? Jump straight into the action:

- **[Installation Guide](getting-started/installation.md)** — Get LLMC running on your machine.
- **[Quickstart](getting-started/quickstart.md)** — Run your first search in under 5 minutes.
- **[Core Concepts](getting-started/concepts.md)** — Understand how LLMC works.

---

## Documentation Sections

The documentation is organized into six main sections based on your needs:

### 🎓 [Getting Started](getting-started/index.md)
**Tutorials & Learning**
Step-by-step lessons to help you get up and running.
- [Installation](getting-started/installation.md)
- [Quickstart](getting-started/quickstart.md)
- [Your First Project](getting-started/first-project.md)

### 📖 [User Guide](user-guide/index.md)
**How-To Guides**
Practical steps for everyday tasks and problem-solving.
- [CLI Reference](user-guide/cli-reference.md)
- [Configuration](user-guide/configuration.md)
- [Search & Navigation](user-guide/search/index.md)
- [Enrichment](user-guide/enrichment/index.md)
- [TUI Dashboard](user-guide/tui/index.md)

### 🔧 [Operations](operations/index.md)
**Ops & Administration**
Managing the RAG daemon, monitoring, and integrations.
- [RAG Daemon](operations/daemon.md)
- [Monitoring & Health](operations/monitoring.md)
- [MCP Integration](operations/mcp-integration.md)
- [Backup & Recovery](operations/backup-recovery.md)

### 🏗️ [Architecture](architecture/index.md)
**Explanations & Design**
Deep dives into how LLMC functions under the hood.
- [Architecture Overview](architecture/index.md)
- [RAG Engine](architecture/rag-engine.md)
- [Enrichment Pipeline](architecture/enrichment-pipeline.md)
- [Security Model](architecture/security-model.md)

### 📚 [Reference](reference/index.md)
**Technical Reference**
Precise technical descriptions of APIs, configuration, and tools.
- [CLI Commands](reference/cli/index.md)
- [Configuration Schema](reference/config/index.md)
- [MCP Tools](reference/mcp-tools/index.md)

### 👩‍💻 [Development](development/index.md)
**Contributing**
Guides for developers working on the LLMC codebase.
- [Contributing Guide](development/contributing.md)
- [Testing](development/testing.md)
- [Agent Contracts](development/agent-contracts.md)

---

## 📋 [Planning](planning/index.md)

Roadmaps, design documents (SDDs), and project management.
- [Roadmap](planning/roadmap.md)
- [Active SDDs](planning/sdd/index.md)
- [Completed Work](planning/completed.md)

---

## Quick Links

| I want to... | Go to... |
|--------------|----------|
| **Install LLMC** | [Installation Guide](getting-started/installation.md) |
| **Run a search** | [Quickstart](getting-started/quickstart.md) |
| **Configure LLMC** | [Configuration](user-guide/configuration.md) |
| **Use Claude Desktop** | [MCP Integration](operations/mcp-integration.md) |
| **Contribute** | [Contributing](development/contributing.md) |

---

## Community & Support

- **GitHub Repository:** [vmlinuzx/llmc](https://github.com/vmlinuzx/llmc)
- **Issue Tracker:** [Report a Bug](https://github.com/vmlinuzx/llmc/issues)
- **Releases:** [CHANGELOG](../CHANGELOG.md)

*Documentation Version: v0.7.0*