...
#### On‑Demand Tool Discovery (Desktop Commander)

When Claude (Otto) needs information about available tools, emit a JSON tool call in a fenced ```json block. The wrapper will execute it and append a `[Tool Result]` section to the transcript. This avoids streaming the full tool manifest into context.

Examples:

```json
{"tool":"search_tools","arguments":{"query":"search, crawl, scrape"}}
```

```json
{"tool":"describe_tool","arguments":{"name":"rg"}}
```

Notes:
- Backed by `.codex/tools.json` and `scripts/tool_query.py` (MCP‑lite).
- Also recognized in inline form: `search_tools("…")`, `describe_tool("…")`.
- See AGENTS.md and CLAUDE_AGENTS.md for persona‑specific guidance.
