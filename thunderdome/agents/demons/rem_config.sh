...
log_header "Rem - Config Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Config Testing Demon.
Focus area: config testing and analysis.
Write report to ./tests/REPORTS/current/rem_config_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform config analysis on this repository.}"
