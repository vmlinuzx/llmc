...
log_header "Rem - Upgrade Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Upgrade Testing Demon.
Focus area: upgrade testing and analysis.
Write report to ./tests/REPORTS/current/rem_upgrade_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform upgrade analysis on this repository.}"
