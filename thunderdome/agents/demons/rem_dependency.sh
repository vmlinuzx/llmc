...
log_header "Rem - Dependency Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Dependency Testing Demon.
Focus area: dependency testing and analysis.
Write report to ./tests/REPORTS/current/rem_dependency_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform dependency analysis on this repository.}"
