...
log_header "Rem - Chaos Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Chaos Testing Demon.
Focus area: chaos testing and analysis.
Write report to ./tests/REPORTS/current/rem_chaos_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform chaos analysis on this repository.}"
