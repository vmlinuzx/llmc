#!/usr/bin/env bash
# Rem - Documentation Demon (Gemini)
set -euo pipefail
...
log_header "Rem - Documentation Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Documentation Testing Demon.
Focus area: documentation testing and analysis.
Write report to ./tests/REPORTS/current/rem_documentation_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform documentation analysis on this repository.}"
