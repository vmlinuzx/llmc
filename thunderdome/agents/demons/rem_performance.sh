...
log_header "Rem - Performance Demon"
log_info "Target: $TARGET_REPO"
init_report_dirs "$TARGET_REPO"
cd "$TARGET_REPO"

PROMPT="You are Rem the Performance Testing Demon.
Focus area: performance testing and analysis.
Write report to ./tests/REPORTS/current/rem_performance_$(date +%Y-%m-%d).md

Context:
$(repo_snapshot "$TARGET_REPO")

Task: ${USER_PROMPT:-Perform performance analysis on this repository.}"
