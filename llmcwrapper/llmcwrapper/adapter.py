...
def _invariants(mode: str, resolved: dict[str, Any], force: bool = False):
    profile = resolved.get("defaults", {}).get("profile") or resolved.get("__run__", {}).get(
        "profile"
    )
...
def send(
    ...
    *,
    ...
    resolved = load_resolved_config(
        profile=profile, mode=mode, overlays=overlays, sets=sets, unsets=unsets
    )
    ...
    if tools and not caps.get("tools", False):
        if not force:
            raise AdapterError(
                f"Provider '{provider}' does not support tools (pass --force to override)"
            )

    if dry_run:
        log_event(".", corr_id, "dry_run", {"provider": provider, "model": model_name})
        return {
            "message": {"role": "assistant", "content": "[dry-run] no call executed"},
    ...
    est = estimate_cost(
        provider, model_name, usage.get("input_tokens"), usage.get("output_tokens"), price_cfg
    )
