...
2. **Bootstrap LazyVim:**
   ```bash
   git clone https://github.com/LazyVim/starter ~/.llmc/config/nvim
   cd ~/.llmc/config/nvim && rm -rf .git && nvim
   ```
...
Run the Python CLI under `llmc/tools/rag/` to build and maintain the `.llmc/.rag/` cache:
...
# full index (writes .llmc/.rag/index_v2.db and versioned spans.jsonl)
...
Outputs live in `.llmc/.rag/` (ignored by git): SQLite DB with span metadata and an optional JSONL export per run. Span hashes are stable per language + code bytes, so enrichment/embedding layers can skip repeats.

Helper script: `llmc/scripts/rag_sync.sh` accepts file paths (relative or absolute inside the repo) and forwards them to `rag sync --stdin`. Editor integrations call this to avoid retyping commands.
...
See `DOCS/SETUP/Editor_Hooks.md` for LazyVim and VS Code snippets that call `llmc/scripts/rag_sync.sh`, which in turn drives `rag sync --stdin`. These hooks keep the index warm without manual commands.
...
1. **Broaden span extraction** (Go/Java/C/C++/Rust/etc.) by adding language-specific walkers in `llmc/tools/rag/lang.py`.
...
4. **Deterministic agents:** build enrichment/embedding workers that read `.llmc/.rag/index.db` and write back summaries/tags before escalating to Qwen.
