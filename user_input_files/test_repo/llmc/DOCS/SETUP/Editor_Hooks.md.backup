...
- You have a virtualenv with the dependencies from `llmc/tools/rag/requirements.txt` (export `RAG_VENV=/path/to/venv`).
- The helper script `llmc/scripts/rag_sync.sh` is executable and lives in the repo root (added in this change).

## LazyVim / Neovim

Create `~/.llmc/config/nvim/lua/plugins/rag.lua` (or add to an existing LazyVim custom module):
...
Tips:
- Set `RAG_VENV` in your shell or VS Code environment to point to the virtualenv so `llmc/scripts/rag_sync.sh` can locate Python dependencies.
...
These hooks are optional but keep the `.llmc/.rag/index.db` warm for instantaneous lookups and drastically reduce remote token spend.
