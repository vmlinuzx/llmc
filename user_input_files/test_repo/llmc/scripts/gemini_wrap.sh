...
if [ "${LLMC_CONCURRENCY:-off}" = "on" ] && [ -n "${CHANGESET_PATH:-}" ] && [ -f "${CHANGESET_PATH}" ]; then
  ...
  if [ "${LLMC_SHADOW_MODE:-off}" = "on" ]; then
    llmc/scripts/llmc_edit.sh --changeset "${CHANGESET_PATH}" || exit $?
  else
    llmc/scripts/llmc_edit.sh --changeset "${CHANGESET_PATH}" || exit $?
  fi
fi
...
print_gemini_wrap_help() {
  cat <<'EOF'
Usage: llmc/scripts/gemini_wrap.sh [options] [prompt|prompt_file]
...
Examples:
  llmc/scripts/gemini_wrap.sh --local "Fix the failing unit test"
  llmc/scripts/gemini_wrap.sh --repo ../other/repo task.txt
EOF
}
...
# Load ToolCaps state if present (written by llmc/scripts/tool_health.sh)
