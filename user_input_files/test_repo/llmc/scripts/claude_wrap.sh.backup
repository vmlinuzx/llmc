...
print_claude_wrap_help() {
  cat <<'EOF'
Usage: llmc/scripts/claude_wrap.sh [options] [prompt|prompt_file]
...
Examples:
  llmc/scripts/claude_wrap.sh --local "Fix the failing unit test"
  llmc/scripts/claude_wrap.sh --azure "Add user authentication"
  llmc/scripts/claude_wrap.sh --yolo "Deploy everything now!"  # YOLO mode bypasses checks
  llmc/scripts/claude_wrap.sh --dangerously-skip-permissions "Force execute task"
  llmc/scripts/claude_wrap.sh --yolo --local "YOLO with routing"  # Works with other options
  llmc/scripts/claude_wrap.sh --tui minimax  # Launch interactive TUI with MiniMax
  llmc/scripts/claude_wrap.sh --tui claude   # Launch interactive TUI with Claude
  llmc/scripts/claude_wrap.sh --tui azure    # Launch interactive TUI with Azure
  llmc/scripts/claude_wrap.sh --template     # Launch Template Builder TUI
  llmc/scripts/claude_wrap.sh --repo ../other/repo task.txt
EOF
}
...
resolve_rag_index_path() {
  ...
  local candidate="${LLMC_RAG_INDEX_PATH:-$ROOT/.llmc/.rag/index_v2.db}"
  if [ -f "$candidate" ]; then
    echo "$candidate"
    return 0
  fi
  candidate="$ROOT/.llmc/.rag/index.db"
  if [ -f "$candidate" ]; then
    echo "$candidate"
    return 0
  fi
...
"
  ...
  if [ "${INCLUDE_AGENTS:-1}" = "1" ]; then
    ...
    if [ -f "$AGENTS" ]; then
      ...
      echo "⚠️  Warning: CLAUDE_AGENTS.md not found at $AGENTS" >&2
    fi
