...
## Behavior
- If a recognized call is found, the dispatcher runs `scripts/tool_query.py` and appends:
...
- Pipe JSON tool call:
  ```bash
  cat <<'EOF' | scripts/tool_dispatch.sh
  ```json
...
- Inline fallback:
  ```bash
  printf 'Use describe_tool("rg").\n' | scripts/tool_dispatch.sh
  ```

## Integration
- `scripts/codex_wrap.sh` post‑processes model output through this dispatcher before printing/caching. No API keys required.

## Notes
- Dispatcher is intentionally strict‑ish for the JSON path and relaxed for inline patterns to keep it robust.
- Extend by adding new cases in `scripts/tool_dispatch.sh` that invoke local scripts or MCP endpoints.
